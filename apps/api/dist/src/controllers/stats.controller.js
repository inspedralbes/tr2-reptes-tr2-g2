 ...
 */
const getPopularWorkshops = async (req, res) => {
    try {
        const { db } = await (0, mongodb_1.connectToDatabase)();
...
const getAdvancedSearch = async (req, res) => {
    try {
        ...
        if (types) {
            const typeList = typeof types === 'string' ? types.split(',') : types;
 ...
 */
const queryByStep = async (req, res) => {
    try {
        const { db } = await (0, mongodb_1.connectToDatabase)();
...
exports.queryByStep = queryByStep;
/**
 * PATCH /stats/checklist/:id/step
 * REQUISITO: Actualizaciones con operadores ($set, $push)
 */
const addChecklistStep = async (req, res) => {
    try {
        const { db } = await (0, mongodb_1.connectToDatabase)();
        const { id } = req.params;
        const { pas_nom } = req.body;
        const result = await db.collection('request_checklists').updateOne({ id_peticio: parseInt(id) }, {
            $push: {
...
exports.addChecklistStep = addChecklistStep;
/**
 ...
 */
const bookWorkshopPlace = async (req, res) => {
    try {
        const { db } = await (0, mongodb_1.connectToDatabase)();
        const { id } = req.params;
...
exports.bookWorkshopPlace = bookWorkshopPlace;
/**
